...
├── data/
│   ├── raw/
│   │   └── hospital_visits.csv          ← place your dataset here (immutable)
│   ├── processed/                       ← cleaned/engineered datasets (generated)
│   └── external/                        ← third-party / reference datasets
│
├── notebooks/
│   └── 01_eda.ipynb                     ← EDA and visualisation (exploration only)
│
├── src/
...
│
├── logs/
│   └── experiment_log.csv              ← one row per training run (auto-created)
│
